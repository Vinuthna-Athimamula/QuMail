...
export default function SentPage({ user, onLogout }) {
  const { messages, selectedMessage, setSelectedId, loading, error, refresh } = useMailboxFolder(user, 'SENT');
  ...
  const leftContent = (
    <>
      ...
      <div className="mail-list">
        ...
        {!loading && messages.length === 0 && <div className="loading-text">No messages found for Sent.</div>}
  ...
  const rightContent = selectedMessage ? (
    ...
    <div className="loading-text">Select a sent message to view details.</div>
  );
