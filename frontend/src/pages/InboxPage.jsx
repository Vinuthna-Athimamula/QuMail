...
export default function InboxPage({ user, onLogout }) {
  const {
    ...
    error,
    refresh,
  } = useMailboxFolder(user, 'INBOX');

  const needsGmailConnect = /no connected gmail account/i.test(error || '');
  ...
  const leftContent = (
    <>
      {error && !needsGmailConnect && <div className="inline-error">{error}</div>}
      {needsGmailConnect && (
        <div className="loading-text">
          Connect your Gmail account using the button near Compose to load your last 7 days of emails.
        </div>
      )}
      <div className="mail-list">
        {loading && <div className="loading-text">Loading messages...</div>}
        {!loading && messages.length === 0 && !needsGmailConnect && <div className="loading-text">No messages found for Inbox.</div>}
  ...
  const rightContent = selectedMessage ? (
    ...
    <div className="loading-text">Select a message to view details.</div>
  );
  ...
  return (
    <WorkspaceShell
      ...
      badges={{ inbox: messages.filter((message) => message.is_unread).length }}
    />
  );
}
